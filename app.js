...
function normalizeSkill(x={}){
  const out={section:'SKILL 1',name:'新技能',sin:'Lust',type:'斩击',offense:'+0',capacity:'1',base:'4',coins:['+4'],size:'compact',defense:false,effects:'',frame:'auto',icon:'',defenseType:'',iconX:null,iconY:null,iconScale:null,...x};
  ...
  const tier=Math.min(3,Math.max(1,parseInt((String(out.section).match(/\d+/)||['1'])[0])));
  if(out.iconX==null || out.iconX==='') out.iconX=out.defense?3:(tier===1?2:0);
  if(out.iconY==null || out.iconY==='') out.iconY=out.defense?1:(tier===1?1:0);
  if(out.iconScale==null || out.iconScale==='') out.iconScale=out.defense?0.96:1;
  out.iconX=Number(out.iconX)||0;
  out.iconY=Number(out.iconY)||0;
  out.iconScale=Math.max(.75,Math.min(1.2,Number(out.iconScale)||1));
  return out;
}
